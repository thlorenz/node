...
function onlistening() {
  ...
  assert.strictEqual(
    hooks.activitiesOfTypes('SENDWRAP').length, 0,
    'no sendwrap after sock connected and sock.send called');
}

function onsent() {
  const as = hooks.activitiesOfTypes('SENDWRAP');
  send = as[0];

  assert.strictEqual(as.length, 1,
                     'one SENDWRAP created synchronously when message sent');
  assert.strictEqual(send.type, 'SENDWRAP', 'send wrap');
