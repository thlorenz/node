...
const { emitInit, emitBefore, emitAfter, emitDestroy,
        initTriggerId } = require('async_hooks');
// Grab the constants necessary for working with internal arrays.
const { kInit, kBefore, kAfter, kDestroy,
        kAsyncUidCntr } = async_wrap.constants;
...
function addTimeoutIds(item) {
  item._asyncId = ++async_uid_fields[kAsyncUidCntr];
  item._triggerId = initTriggerId();
  if (async_hook_fields[kInit] > 0)
    emitInit(item._asyncId, 'Timeout', this._triggerId, this);
}
...
function listOnTimeout() {
  ...
  while (timer = L.peek(list)) {
    ...
    if (!timer._onTimeout) {
      if (timer._asyncId > 0 && async_hook_fields[kDestroy])
        emitDestroy(timer._asyncId);
      timer._asyncId = timer._triggerId = -1;
      continue;
    }
    ...
    // TODO(trevnorris): The check for asyncId shouldn't be necessary, but
    // because Socket has a custom implementation of _onTimeout() and this is
    // not yet implemented correctly we'll skip those for now.
    ...
    if (timer._asyncId > 0 && async_hook_fields[kBefore]) {
      emitBefore(timer._asyncId, timer._triggerId);
    }
...
const unenroll = exports.unenroll = function(item) {
  if (item._asyncId > 0) {
    ...
    item._asyncId = item._triggerId = -1;
  }
...
function processImmediate() {
  ...
  while (immediate) {
    ...
    if (immediate._asyncId > 0 && async_hook_fields[kBefore]) {
      emitBefore(immediate._asyncId, immediate._triggerId);
    }
    ...
    if (immediate._asyncId > 0 && async_hook_fields[kAfter]) {
      emitAfter(immediate._asyncId);
    }

    if (immediate._asyncId > 0) {
      if (async_hook_fields[kDestroy]) {
        emitDestroy(immediate._asyncId);
      }
      immediate._asyncId = immediate._triggerId = -1;
    }
...
function Immediate() {
  ...
  this._asyncId = ++async_uid_fields[kAsyncUidCntr];
  this._triggerId = initTriggerId();
  if (async_hook_fields[kInit])
    emitInit(this._asyncId, 'Immediate', this._triggerId, this);
}
