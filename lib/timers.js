...
const { kInit, kBefore, kAfter, kDestroy, kCurrentId, kTriggerId,
...
function insert(item, unrefed) {
  ...
  if (!item._asyncId) {
    ...
    if (async_hook_fields[kInit] > 0)
      // TODO(trevnorris): Is the "type" of 'Timeout' appropriate here?
      emitInit(item._asyncId, 'Timeout', item._triggerId, item);
  }
...
function listOnTimeout() {
  ...
  while (timer = L.peek(list)) {
    ...
    async_uid_fields[kCurrentId] = timer._asyncId;
    async_uid_fields[kTriggerId] = timer._triggerId;
...
function Timeout(after, callback, args) {
  ...
  if (async_hook_fields[kInit] > 0)
    emitInit(this._asyncId, 'Timeout', this._triggerId, this);
}
...
Timeout.prototype.close = function() {
  ...
  // TODO(trevnorris): should also emitDestroy() here if necessary.
  if (this._handle) {
