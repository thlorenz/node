...
(function(process) {
  ...
  function setupProcessFatal() {
    ...
    const { async_uid_fields, current_trigger_id_stack,
            trigger_scope_stack } = async_wrap;
    const { kAsyncUidCntr, kCurrentId, kTriggerId } = async_wrap.constants;

    process._fatalException = function(er) {
      var caught;

      async_uid_fields[kAsyncUidCntr] = 0;
      async_uid_fields[kCurrentId] = 0;
      async_uid_fields[kTriggerId] = 0;
      current_trigger_id_stack.length = 0;
      trigger_scope_stack.length = 0;
