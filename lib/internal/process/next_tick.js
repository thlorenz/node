...
function setupNextTick() {
    ...
    do {
      while (tickInfo[kIndex] < tickInfo[kLength]) {
        ...
        if (has_hooks && async_hook_fields[kBefore])
          async_hooks.emitBefore(tock.asyncId, tock.triggerId);
        ...
        if (has_hooks) {
          ...
          if (async_hook_fields[kDestroy])
            async_hooks.emitDestroy(tock.asyncId);
        }
  ...
  function TickObject(callback, args, domain, isMicrotask) {
    ...
    this.domain = domain;
    this.args = args;
    // Done like this to signal these properties should be boxed by default.
    this.asyncId = NaN;
    this.triggerId = NaN;
    if (!isMicrotask)
      setupInit(this);
  ...
  function setupInit(self) {
    ...
    self.asyncId = ++async_uid_fields[kAsyncUidCntr];
    self.triggerId = tId;
    if (async_hook_fields[kInit] > 0)
      async_hooks.emitInit(self.asyncId, 'TickObject', self.triggerId, self);
  }
