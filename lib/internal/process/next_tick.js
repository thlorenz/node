...
function setupNextTick() {
  ...
  function setupInit(self, triggerId) {
    self[async_id_symbol] = ++async_uid_fields[kAsyncUidCntr];
    self[trigger_id_symbol] = triggerId || initTriggerId();
    if (async_hook_fields[kInit] > 0) {
      emitInit(
        self[async_id_symbol], 'TickObject', self[trigger_id_symbol], self);
    }
  }
  ...
  function nextTick(callback) {
    ...
    const obj = new TickObject(callback, args, process.domain || null);
    setupInit(obj, null);
