...
class AsyncHook {
  ...
  enable() {
    ...
    // If this hook is being enabled while in the middle of processing the
    // array of currently active hooks then duplicate the current set of
    // active hooks and store this there. This shouldn't fire until the next
    // time hooks are processed.
    if (processing_hooks) {
...
function asyncIdScope(asyncId, cb) {
...
function setInitTriggerId(asyncId) {
  // CHECK(Number.isSafeInteger(asyncId)
  // CHECK(asyncId > 0)
  async_uid_fields[kInitTriggerId] = asyncId;
}
...
function emitBeforeS(asyncId, triggerId) {
