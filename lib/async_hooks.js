...
class AsyncEvent {
  constructor(type, triggerId) {
    ...
    // Must do this before the early return check to zero out the potentially
    // set kInitTriggerId field.
    ...
    // TODO(trevnorris): Will have issues if the user disables a hook while the
    // active_hooks_array is being processed. So figure out way to maintain the
    // shape of the array while callbacks are being called to make sure none
    // are skipped over.
    // - set flag that array is being processed, in which case the value is set
    //   to null instead of spliced out. Then remove those from the array when
    //   the for loop is complete. though this may conflict with the
    //   kActiveHooks value. Which is main issue. Also need to have proper
    //   error handling to make sure the active_hooks_array is cleaned up.
    //   Possibly don't change kActiveHooks until the array cleanup after for
    //   loop is complete, and use kActiveHooks for the for loop length so
    //   adding hooks to the array won't be executed.
  ...
  emitBefore() {
    emitBeforeS(this[async_id_symbol]);
    return this;
  }
...
function initTriggerId() {
  if (async_uid_fields[kInitTriggerId] > 0) {
    const val = async_uid_fields[kInitTriggerId];
  ...
  if (async_uid_fields[kScopedTriggerId] > 0)
    return async_uid_fields[kScopedTriggerId];
  return async_uid_fields[kCurrentId];
}
...
function emitInitS(id, type, triggerId, handle) {
  ...
  // TODO(trevnorris): Will have issues if the user disables a hook while the
  // active_hooks_array is being processed. So figure out way to maintain the
  // shape of the array while callbacks are being called to make sure none are
  // skipped over.
  // - set flag that array is being processed, in which case the value is set
  //   to null instead of spliced out. Then remove those from the array when
  //   the for loop is complete. though this may conflict with the kActiveHooks
  //   value. Which is main issue. Also need to have proper error handling to
  //   make sure the active_hooks_array is cleaned up. Possibly don't change
  //   kActiveHooks until the array cleanup after for loop is complete, and
